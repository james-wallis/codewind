...
async function getTypes(templates, providers, extensionList) {
  const projectTypes = [];
  ...
  for (const template of templates) {
    ...
    if (extension) {
      ...
      if (seenProjectTypes[key])
        continue;
      console.log(providers[extension.name]);
      
      const types = await getProjectTypes(providers[extension.name], sourceId);
